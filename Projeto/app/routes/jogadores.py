...
async def salvar_adicao_jogador(
    ...
    novo_jogador = models.Jogador(nome=nome, posicao=posicao, time_id=time_id)
    db.add(novo_jogador)
    db.commit()
...
# Rota HTML: Excluir Jogador
@router.post("/deletar/{jogador_id}")
async def deletar_jogador(jogador_id: int, db: Session = Depends(get_db)):
    jogador = db.query(models.Jogador).filter(models.Jogador.id == jogador_id).first()
    if not jogador:
        raise HTTPException(
            status_code=404, detail="Jogador não encontrado para exclusão."
        )
    db.delete(jogador)
    db.commit()
    return RedirectResponse(url="/jogadores", status_code=303)
